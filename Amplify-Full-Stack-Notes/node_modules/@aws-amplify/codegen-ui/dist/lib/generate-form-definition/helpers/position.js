"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.mapElementMatrix = exports.removeAndReturnItemOnward = exports.removeFromMatrix = exports.findIndices = void 0;
/*
  Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.

  Licensed under the Apache License, Version 2.0 (the "License").
  You may not use this file except in compliance with the License.
  You may obtain a copy of the License at

      http://www.apache.org/licenses/LICENSE-2.0

  Unless required by applicable law or agreed to in writing, software
  distributed under the License is distributed on an "AS IS" BASIS,
  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  See the License for the specific language governing permissions and
  limitations under the License.
 */
const errors_1 = require("../../errors");
/**
 * Helper that returns the row and column indices of a string in a two-dimensional array
 */
const findIndices = (name, matrix) => {
    for (let rowIndex = 0; rowIndex < matrix.length; rowIndex += 1) {
        const columnIndex = matrix[rowIndex].indexOf(name);
        if (columnIndex !== -1) {
            return [rowIndex, columnIndex];
        }
    }
    return undefined;
};
exports.findIndices = findIndices;
function removeFromMatrix(indices, formDefinition) {
    formDefinition.elementMatrix[indices[0]].splice(indices[1], 1);
}
exports.removeFromMatrix = removeFromMatrix;
function removeAndReturnItemOnward(indices, formDefinition) {
    const row = formDefinition.elementMatrix[indices[0]];
    return row.splice(indices[1], row.length - indices[1]);
}
exports.removeAndReturnItemOnward = removeAndReturnItemOnward;
// helper that throws if every element traversed but none mapped
function throwIfUnmappable(originalQueue, requeued) {
    if (originalQueue.length === requeued.length) {
        throw new errors_1.InvalidInputError('Unmappable element positions in form');
    }
}
/* eslint-disable no-param-reassign */
/**
 * Impure function that maps the element matrix of form definition
 * from a queue of sectional and input elements.
 */
function mapElementMatrix({ elementQueue, formDefinition, }) {
    let belowElementQueue = [...elementQueue];
    let rightOfElementQueue = [];
    // map elements with no position; position below; position fixed to first
    while (belowElementQueue.length) {
        const requeued = [];
        const tempRightOf = [];
        belowElementQueue.forEach((element) => {
            if (element.excluded) {
                const previousIndices = (0, exports.findIndices)(element.name, formDefinition.elementMatrix);
                if (previousIndices) {
                    removeFromMatrix(previousIndices, formDefinition);
                }
            }
            else if (typeof element.position === 'object' && 'rightOf' in element.position && element.position.rightOf) {
                tempRightOf.push(element);
            }
            else if (typeof element.position === 'object' && 'below' in element.position && element.position.below) {
                const relationIndices = (0, exports.findIndices)(element.position.below, formDefinition.elementMatrix);
                const previousIndices = (0, exports.findIndices)(element.name, formDefinition.elementMatrix);
                if (previousIndices) {
                    removeFromMatrix(previousIndices, formDefinition);
                }
                if (!relationIndices) {
                    requeued.push(element);
                }
                else {
                    formDefinition.elementMatrix.splice(relationIndices[0] + 1, 0, [element.name]);
                }
            }
            else if (typeof element.position === 'object' &&
                'fixed' in element.position &&
                element.position.fixed === 'first') {
                const previousIndices = (0, exports.findIndices)(element.name, formDefinition.elementMatrix);
                if (previousIndices) {
                    removeFromMatrix(previousIndices, formDefinition);
                }
                formDefinition.elementMatrix.unshift([element.name]);
            }
            else {
                const previousIndices = (0, exports.findIndices)(element.name, formDefinition.elementMatrix);
                if (!previousIndices) {
                    formDefinition.elementMatrix.push([element.name]);
                }
            }
        });
        throwIfUnmappable(belowElementQueue, requeued);
        belowElementQueue = requeued;
        rightOfElementQueue.push(...tempRightOf);
    }
    // map elements with rightOf position
    while (rightOfElementQueue.length) {
        const requeued = [];
        rightOfElementQueue.forEach((element) => {
            if (typeof element.position === 'object' && 'rightOf' in element.position && element.position.rightOf) {
                const relationIndices = (0, exports.findIndices)(element.position.rightOf, formDefinition.elementMatrix);
                if (!relationIndices) {
                    requeued.push(element);
                }
                else {
                    const previousIndices = (0, exports.findIndices)(element.name, formDefinition.elementMatrix);
                    if (previousIndices) {
                        const removedItems = removeAndReturnItemOnward(previousIndices, formDefinition);
                        formDefinition.elementMatrix[relationIndices[0]].splice(relationIndices[1] + 1, 0, ...removedItems);
                    }
                    else {
                        formDefinition.elementMatrix[relationIndices[0]].splice(relationIndices[1] + 1, 0, element.name);
                    }
                }
            }
        });
        throwIfUnmappable(rightOfElementQueue, requeued);
        rightOfElementQueue = requeued;
    }
    // filter out empty rows
    formDefinition.elementMatrix = formDefinition.elementMatrix.filter((row) => row.length);
}
exports.mapElementMatrix = mapElementMatrix;
/* eslint-enable no-param-reassign */
//# sourceMappingURL=position.js.map