...
exports.compileToLegacyIR = void 0;
const _1 = require("./");
...
function compileToLegacyIR(schema, document, options = { mergeInFieldsFromFragmentSpreads: true }) {
    const context = (0, _1.compileToIR)(schema, document, options);
...
class LegacyIRTransformer {
    ...
    transformIR() {
        ...
        for (const [operationName, operation] of Object.entries(this.context.operations)) {
            ...
            const fragmentsReferenced = (0, collectFragmentsReferenced_1.collectFragmentsReferenced)(selectionSet, this.context.fragments);
            const { sourceWithFragments, operationId } = (0, generateOperationId_1.generateOperationId)(operation, this.context.fragments, fragmentsReferenced);
    ...
    transformSelectionSetToLegacyIR(selectionSet) {
        const typeCase = (0, typeCase_1.typeCaseForSelectionSet)(selectionSet, this.options.mergeInFieldsFromFragmentSpreads);
        const fields = this.transformFieldsToLegacyIR((0, collectAndMergeFields_1.collectAndMergeFields)(typeCase.default, false));
        const inlineFragments = typeCase.variants.flatMap(variant => {
            const fields = this.transformFieldsToLegacyIR((0, collectAndMergeFields_1.collectAndMergeFields)(variant, false));
