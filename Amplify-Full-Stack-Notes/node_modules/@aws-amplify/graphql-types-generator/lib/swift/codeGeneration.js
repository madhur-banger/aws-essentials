...
exports.SwiftAPIGenerator = exports.generateSource = void 0;
...
const appSyncCompatibilityTypes_1 = require("./appSyncCompatibilityTypes");
...
class SwiftAPIGenerator extends language_1.SwiftGenerator {
    ...
    fileHeader() {
        ...
        this.print(appSyncCompatibilityTypes_1.appSyncCompatibilityTypesCode);
    ...
    classDeclarationForOperation(operation) {
        ...
        this.classDeclaration({
            ...
            if (variables && variables.length > 0) {
                ...
                this.withinBlock(() => {
                    this.printOnNewline((0, printing_1.wrap)(`return [`, (0, printing_1.join)(properties.map(({ name, propertyName }) => `"${name}": ${(0, language_1.escapeIdentifierIfNeeded)(propertyName)}`), ', ') || ':', `]`));
                });
            }
            else {
    ...
    structDeclarationForSelectionSet({ structName, adoptedProtocols = ['GraphQLSelectionSet'], selectionSet, }, before) {
        const typeCase = (0, typeCase_1.typeCaseForSelectionSet)(selectionSet, this.context.options.mergeInFieldsFromFragmentSpreads);
    ...
    structDeclarationForVariant({ structName, adoptedProtocols = ['GraphQLSelectionSet'], variant, typeCase, }, before, after) {
        this.structDeclaration({ structName, adoptedProtocols }, () => {
            ...
            this.print((0, printing_1.join)(variant.possibleTypes.map(type => `"${type.name}"`), ', '));
            this.print(']');
            this.printNewlineIfNeeded();
            ...
            const fields = (0, collectAndMergeFields_1.collectAndMergeFields)(variant, this.context.options.mergeInFieldsFromFragmentSpreads).map(field => this.helpers.propertyFromField(field));
            ...
            for (const field of fields) {
                if ((0, graphql_1.isCompositeType)((0, graphql_1.getNamedType)(field.type)) && field.selectionSet) {
                    this.structDeclarationForSelectionSet({
    ...
    initializersForVariant(variant, namespace, useInitializerIfPossible = true) {
        if (useInitializerIfPossible && variant.possibleTypes.length == 1) {
            ...
            this.withinBlock(() => {
                this.printOnNewline((0, printing_1.wrap)(`self.init(snapshot: [`, (0, printing_1.join)([`"__typename": "${variant.possibleTypes[0]}"`, ...properties.map(this.propertyAssignmentForField, this)], ', ') || ':', `])`));
            });
        }
        else {
    ...
    initializerDeclarationForProperties(properties) {
        ...
        this.withinBlock(() => {
            properties.forEach(({ propertyName }) => {
                this.printOnNewline(`self.${propertyName} = ${(0, language_1.escapeIdentifierIfNeeded)(propertyName)}`);
            });
        });
    }
    parametersForProperties(properties) {
        this.print('(');
        this.print((0, printing_1.join)(properties.map(({ propertyName, typeName, isOptional }) => (0, printing_1.join)([`${(0, language_1.escapeIdentifierIfNeeded)(propertyName)}: ${typeName}`, isOptional && ' = nil'])), ', '));
        this.print(')');
    }
            ...
            for (const selection of selectionSet.selections) {
                switch (selection.kind) {
                    ...
                    case 'BooleanCondition':
                        ...
                        this.print((0, printing_1.join)([`variableName: "${selection.variableName}"`, `inverted: ${selection.inverted}`, 'selections: '], ', '));
                        this.selectionSetInitialization(selection.selectionSet);
                        this.print('),');
                        break;
                    case 'TypeCondition': {
                        this.printOnNewline(`GraphQLTypeCondition(`);
                        this.print((0, printing_1.join)([`possibleTypes: [${(0, printing_1.join)(selection.selectionSet.possibleTypes.map(type => `"${type.name}"`), ', ')}]`, 'selections: '], ', '));
                        this.selectionSetInitialization(selection.selectionSet);
                        this.print('),');
                        break;
                    }
    ...
    structDeclarationForInputObjectType(type) {
        ...
        if ((0, complextypes_1.isS3Field)(type)) {
            properties = [
                ...properties,
        ...
        this.structDeclaration({ structName, description: description || undefined, adoptedProtocols }, () => {
            ...
            this.print(')');
            this.withinBlock(() => {
                this.printOnNewline((0, printing_1.wrap)(`graphQLMap = [`, (0, printing_1.join)(properties.map(({ name, propertyName }) => `"${name}": ${(0, language_1.escapeIdentifierIfNeeded)(propertyName)}`), ', ') || ':', `]`));
            });
