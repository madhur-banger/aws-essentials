...
function transformComment(comment, indentLevel = 0) {
    ...
    return lines
        ...
        if (isFirst && isLast) {
            return (0, visitor_plugin_common_1.indent)(`// ${comment} */\n`, indentLevel);
        }
        return (0, visitor_plugin_common_1.indent)(`${isFirst ? '/*' : ''} * ${line}${isLast ? '\n */\n' : ''}`, indentLevel);
    })
        .join('\n');
}
class TypeScriptDeclarationBlock {
    ...
    generateEnum() {
        ...
        const body = Object.entries(this._enumValues)
            ...
            .join(',\n');
        return [`${header} {`, (0, visitor_plugin_common_1.indentMultiline)(body), '}'].join('\n');
    }
    generateClass() {
        const header = [
        ...
        return [`${header.filter(h => h).join(' ')}`, (0, visitor_plugin_common_1.indentMultiline)(body.join('\n')), '}'].join('\n');
    }
    generateProperties() {
    ...
    generateMethods() {
        const methods = this._methods.map(method => {
            ...
            return [`${methodHeaderStr}`, '{', method.implmentation ? (0, visitor_plugin_common_1.indentMultiline)(method.implmentation) : '', '}'].join('\n');
        });
        return methods.join('\n');
    }
    generateInterface() {
